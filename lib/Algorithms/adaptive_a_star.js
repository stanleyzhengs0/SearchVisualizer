...
export function adaptive_a_star(grid, startNode, finishnode){
    let agentsGrid = grid;
    let expansions = 0;
    let new_start = startNode; 
        ...
        let filteredArrayOfObjects = recontructedPath.filter(obj => !obj.isWall);
        //console.log(filteredArrayOfObjects, "recontructedPath PATH")
...
export function visitedAdaptive(){
return visitedNodes
}
...
function updateHcost(){
    for(var i = 0; i < closedList.length; i++){
        if(closedList[i].h_cost < finished[i].g_cost - closedList[i].g_cost){
            closedList[i].h_cost = finished[i].g_cost - closedList[i].g_cost
        }
    }
}
