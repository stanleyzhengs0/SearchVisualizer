...
export function backwards_a_star(grid, startNode, finishnode){
    ...
    let counter = 0
        while(startNode != finishnode){
            ...
            [partialPath, hitWall, isFinished] = agentPath(optimalPath)

            console.log(partialPath,"PP" , hitWall)

            //rebuild path
            reconstructedPath.push(...partialPath);

            
            console.log(reconstructedPath, "Recont")
            console.log(visitedBlocks)

            if(isFinished){return reconstructedPath; }
            ...
            if(hitWall){
                ...
                console.log(new_finish, "NEWfinsidA")
...
export function a_star(grid, startNode, finishNode, expansions, visitedBlocks){
    ...
    while(startNode != finishNode){
        ...
        //if(currentNode.isWall){ return foundPath(currentNode); }
...
export function visitedBackwards(){
return visitedNodes
}
...
function resetAllNodes(grid, newFinish){
